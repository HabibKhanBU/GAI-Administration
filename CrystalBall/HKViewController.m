...
- (void)viewDidLoad
{
   ...
   // NSString *soundPath = [[NSBundle mainBundle] pathForResource:@"cyrstal_ball" ofType:@"mp3"];
    
    NSString *soundPath = [[NSString alloc] initWithString:[[NSBundle mainBundle] pathForResource:@"crystal_ball" ofType:@"mp3"]];
    NSURL *soundURL = [NSURL fileURLWithPath:soundPath];
    AudioServicesCreateSystemSoundID(CFBridgingRetain(soundURL), &soundEffect);
    
    self.crystalBall = [[HKCrystalBall alloc] init];
...
//    Code below programmatically adds background.
...
- (void) makePrediction
{
    ...
    self.predictionText.text = [self.crystalBall randomPrediction];
    
    AudioServicesPlaySystemSound(soundEffect);

    [UIView animateWithDuration: 6.0 animations:^{
        self.predictionText.alpha = 1.0f;
    }];
...
{
    self.predictionText.text = nil;
    self.predictionText.alpha = 0.0f;
}
